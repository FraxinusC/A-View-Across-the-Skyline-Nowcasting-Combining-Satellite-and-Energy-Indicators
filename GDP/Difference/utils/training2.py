...
def train_diff_model(model, train_loader, val_loader, optimizer, scheduler, criterion, device, num_epochs=500, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        for img1, img2, cur_econ, prev_econ, econ_diff, targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            img1, img2 = img1.to(device), img2.to(device)
            cur_econ = cur_econ.to(device)
            prev_econ = prev_econ.to(device)
            econ_diff = econ_diff.to(device)
            targets = targets.float().unsqueeze(1).to(device)

            optimizer.zero_grad()
            outputs = model(img1, img2, cur_econ, prev_econ, econ_diff)
        ...
        print(f"[Epoch {epoch+1}] Train Loss: {train_loss:.4f}, R²: {train_r2:.4f} | Val Loss: {val_loss:.4f}, R²: {val_r2:.4f}")
...
def evaluate_diff_model(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for img1, img2, cur_econ, prev_econ, econ_diff, targets in loader:
            img1, img2 = img1.to(device), img2.to(device)
            cur_econ = cur_econ.to(device)
            prev_econ = prev_econ.to(device)
            econ_diff = econ_diff.to(device)
